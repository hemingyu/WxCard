...
public class RedPackOAuth extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static Logger log = Logger.getLogger(RedPackOAuth.class.getName());
	*//**
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (code!=null &&!"authdeny".equals(code)) {
			try{
				// 获取网页授权Access_token
				request.getRequestDispatcher("/getRed.jsp?state=1&code="+code).forward(request, response);	    
			}catch(Exception e){
				e.printStackTrace();
				log.info("请求异常");
			}
